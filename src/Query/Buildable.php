...
/**
 * Base class for buildable queries
 *
 * @package    fleks-db
 ...
 */
class Buildable extends AbstractQuery
{
    /**
     * The table
     *
     ...
     * @param string $table The table the columns belong to. Specify '*' if the
     *   column's don't belong to any table. This is also the default value.
     */
     ...
     * @return string The generated parameter name (beginning with a colon)
     */
     ...
     * @param string $glue Either 'AND' or 'OR'. Defaults to 'OR'
     * @return string The part of the 'where' clause
     */
        ...
        foreach ($conditions as $key => $value) {
            if ($key == '$or' || $key == '$and') {
                ...
                if (is_array($value) && !array_key_exists(0, $value)) {
                    ...
                    $operandArr = [];
                    foreach ($value as $v) {
                        $operandArr[] = $this->bindValue($v);
                    }
                    $operand = '(' . implode(', ', $operandArr) . ')';
                } else {
                    $bindValue = $value;
                }
